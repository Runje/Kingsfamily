...
interface StatisticEntry : Byteable {
    ...
    companion object {
        ...
        fun bytesToEntryMap(bytes: ByteArray): HashMap<User, Int> {
            return bufferToEntryMap(ByteBuffer.wrap(bytes))
        }

        fun bufferToEntryMap(buffer: ByteBuffer): HashMap<User, Int> {
 ...
 */
fun yearMonthRange(from: YearMonth, to: YearMonth, stepSizeInMonth: Int = 1): List<YearMonth> {
    val result = mutableListOf<YearMonth>()
    var month = from

    while (month <= to) {
        result.add(month)
        month = month.plusMonths(stepSizeInMonth)
    }
